...
LRESULT CaptureDlg::OnInitDialog( UINT nMessage, WPARAM wParam, LPARAM lParam, BOOL& bHandled )
{
	...
	constexpr uint32_t defaultFlags = (uint32_t)eTextFlags::Save;
...
void CaptureDlg::onSaveTextCheckbox()
{
	...
	std::array<HWND, 4> controls = { checkAppend, checkTimestamps };
	for( HWND w : controls )
...
inline HRESULT CaptureDlg::runCapture()
{
	...
	CString data;
	data.GetBufferSetLength(file->GetLength());
	int len = MultiByteToWideChar(CP_UTF8, MB_PRECOMPOSED, file->GetString(), file->GetLength(), data.GetBuffer(), file->GetLength());
	data.ReleaseBuffer();
	if (append) {
		outdata.Append(data);
	}
...
HRESULT __cdecl CaptureDlg::newSegmentCallback( Whisper::iContext* ctx, uint32_t n_new, void* user_data ) noexcept
{
	...
	auto ret = dialog.appendTextFile( result, n_new );
	return ret;
}
...
HRESULT CaptureDlg::appendTextFile( Whisper::iTranscribeResult* results, uint32_t newSegments )
{
	...
	for( ; i < len; i++ )
	{
		...
		threadState.file->Append(str);
	}
	return S_OK;
}

void CaptureDlg::onSaveFile()
{
	CString outpath;
