...
LRESULT LoadModelDlg::OnInitDialog( UINT nMessage, WPARAM wParam, LPARAM lParam, BOOL& bHandled )
{
	...
	auto exepath = GetModuleDir();
	exepath.Append(L"\\model");
	getAllFiles(exepath.GetString(), pathlist);
	if (pathlist.empty()) {
		::MessageBox(m_hWnd, L"No Model Found.", L"Error", MB_OK | MB_ICONWARNING);
	}
	for (int i = 0; i < pathlist.size(); i++) {
		cbmodelpath.AddString(pathlist[i]);
	}
	cbmodelpath.SetCurSel(0);
...
{
	if (pathlist.empty()) {
		return validationError(L"No model file");
	}
	if (cbmodelpath.GetCurSel()<0 || cbmodelpath.GetCurSel()>=pathlist.size()) {
		return validationError(L"Please select a model");
	}
	auto pmodule = GetModuleDir();
	pmodule.Append(L"\\model\\");
	pmodule.Append(pathlist[cbmodelpath.GetCurSel()]);
	path = pmodule;
	{
		CAtlFile file;
